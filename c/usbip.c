...
void
send_usb_req(int sockfd, USBIP_RET_SUBMIT *usb_req, char *data, unsigned int size, unsigned int status)
{
        usb_req->command = 0x3;
...
handle_get_descriptor(int sockfd, StandardDeviceRequest *control_req, USBIP_RET_SUBMIT *usb_req)
{
	...
	if (control_req->wValue1 == 0x3) {
		...
		len = usb_req->actual_length;
		if (len > *strings[control_req->wValue0])
			len = *strings[control_req->wValue0];
		for (i = 0; i < len / 2 - 1; i++)
			str[i] = strings[control_req->wValue0][i * 2 + 2];
		printf("String (%s)\n",str);
		handled = 1;
		send_usb_req(sockfd,usb_req, (char *)strings[control_req->wValue0], len, 0);
	}
	if (control_req->wValue1 == 0x6) {
		// qualifier
		printf("Qualifier\n");
		handled = 1;
		send_usb_req(sockfd,usb_req, (char *)&dev_qua, control_req->wLength, 0);
	}
	if (control_req->wValue1 == 0xA) {
		// config status ???
		printf("Unknow\n");  
		handled = 1;
...
static int
handle_set_configuration(int sockfd, StandardDeviceRequest *control_req, USBIP_RET_SUBMIT *usb_req)
{
	int handled = 0;
...
static void
handle_usb_control(int sockfd, USBIP_RET_SUBMIT *usb_req)
{
        int	handled = 0;
...
static void
handle_usb_request(int sockfd, USBIP_RET_SUBMIT *ret, int bl)
{
	if (ret->ep == 0) {
...
static int
handle_unattached_devlist(int sockfd, const USB_DEVICE_DESCRIPTOR *dev_dsc)
{
	OP_REP_DEVLIST	list;
...
static int
handle_unattached_import(int sockfd, const USB_DEVICE_DESCRIPTOR *dev_dsc)
{
	char	busid[32];
	OP_REP_IMPORT	rep;
	int	nb;
...
static int
handle_unattached(int sockfd, const USB_DEVICE_DESCRIPTOR *dev_dsc)
{
	OP_REQ_DEVLIST	req;
	int	nb;
	...
	switch (req.command) {
	case 0x8005:
		return handle_unattached_devlist(sockfd, dev_dsc);
	case 0x8003:
		return handle_unattached_import(sockfd, dev_dsc);
	default:
		return -1;
	}
}
...
usbip_run(const USB_DEVICE_DESCRIPTOR *dev_dsc)
{
	...
	for (;;) {
		...
		while (1) {
			if (!attached)
				handle_unattached(sockfd, dev_dsc);
			else {
				if (handle_attached(sockfd) < 0)
					break;
			}
		}
		close(sockfd);
